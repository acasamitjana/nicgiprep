...
parser.add_argument(
    "--bids", default=BIDS_DIR, help="specify the bids root directory (/rawdata)"
)
parser.add_argument(
    "--subjects",
    default=None,
...
parser.add_argument(
    "--gpu",
    action="store_true",
    help="Enable GPU for SuperSynth/SynthSeg (default: GPU/CPU)",
)
parser.add_argument(
    "--gpu_num", default=0, help="GPU number to be used for SuperSynth/SynthSeg"
)
parser.add_argument(
    "--num_cores", default=1, type=int, help="number of parallel cores to run."
)
...
if args.subjects is None:
    print(
        "Total subjects found: N=" + str(len(bids_loader.get_subjects())),
        end=" ",
        flush=True,
    )
else:
