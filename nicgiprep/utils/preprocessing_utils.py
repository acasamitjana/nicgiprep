...
def one_hot_encoding_with_gaussian(
    target: np.ndarray,
    num_classes: int,
    ...
    if sigma == 0:
        ...
        return gaussian_filter(
            labels, sigma
        )  # size of gaussian kernel is 4*sigma + 0.5 at each side and center=2.


def get_dct_basis_functions(
    image_shape: tuple[int, ...], smoothing_kernel_size: list[float]
) -> list[np.ndarray]:
...
def backprojectKroneckerProductBasisFunctions(
    kroneckerProductBasisFunctions: list[np.ndarray], coefficients: np.ndarray
) -> np.ndarray:
    ...
    y = projectKroneckerProductBasisFunctions(
        transposedKroneckerProductBasisFunctions, coefficients.reshape(Ms, order="F")
    )
...
def projectKroneckerProductBasisFunctions(
    kroneckerProductBasisFunctions: list[np.ndarray], T: np.ndarray
) -> np.ndarray:
...
def computePrecisionOfKroneckerProductBasisFunctions(
    kroneckerProductBasisFunctions: list[np.ndarray], B: np.ndarray
) -> np.ndarray:
    ...
    result = projectKroneckerProductBasisFunctions(
...
def fitBiasFieldParameters(
    ...
    bias_field_functions: list[np.ndarray],
    mask: np.ndarray,
    penalty: float = 1,
) -> np.ndarray:
    ...
    for contrastNumber1 in range(numberOfContrasts):
        ...
        for contrastNumber2 in range(numberOfContrasts):
            ...
            classSpecificWeights = (
                soft_seg * precisions[:, contrastNumber1, contrastNumber2]
            )
            weights = np.sum(classSpecificWeights, 1)

            # Build up stuff needed for rhs
            predicted = np.sum(classSpecificWeights * means[:, contrastNumber2], 1) / (
                weights + eps
            )
            ...
            lhs[np.ix_(contrast1Indices, contrast2Indices)] = (
                computePrecisionOfKroneckerProductBasisFunctions(
                    bias_field_functions, weightsImageBuffer
                )
            )

        tmpImageBuffer[mask] = tmp
        rhs[contrast1Indices] = projectKroneckerProductBasisFunctions(
            bias_field_functions, tmpImageBuffer
        ).reshape(-1, 1)
    ...
    #
    biasFieldCoefficients = solution.reshape(
...
def getBiasFields(
    ...
    biasFieldBasisFunctions: list[np.ndarray],
...
def undoLogTransformAndBiasField(
    imageBuffers: np.ndarray, biasFields: np.ndarray, mask: np.ndarray
...
def getGaussianLikelihoods(
    data: np.ndarray, mean: np.ndarray, variance: np.ndarray
) -> np.ndarray:
...
def getGaussianPosteriors(
    data: np.ndarray, classPriors: np.ndarray, means: np.ndarray, variances: np.ndarray
...
def bias_field_corr(
    ...
    if len(init_image.shape) == 3:
        init_image = init_image[..., np.newaxis]
    ...
    image_log = (
        init_image_log  # image_log = image_utils.apply_crop(init_image_log, crop_coord)
    )
    ...
    for it in range(100):
        ...
        u_j = np.sum(image_posteriors * image_log_corr, axis=0) / np.sum(
            image_posteriors, axis=0
        )
        s_j = np.sum(image_posteriors * (image_log_corr - u_j) ** 2, axis=0) / np.sum(
            image_posteriors, axis=0
        )
        u_j = u_j.reshape(-1, 1)
        ...
        coeff = fitBiasFieldParameters(
            image_log, image_posteriors, u_j, s_j, basis_functions, mask, penalty
        )
        ...
        if VERBOSE:
            print(
                "          " + str(it) + ". Loglikelihood improvement: " + str(improv)
            )
        llh_last = llh
        if improv < 1e-3:
            it_break += 1
    ...
    bias_field_log = getBiasFields(coeff, basis_functions)
    image_corr, bias_field = undoLogTransformAndBiasField(
        image_log, bias_field_log, mask
    )
